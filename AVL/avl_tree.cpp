...
void AVL_Tree_Node<Comparable>::remove(const Comparable &x, Node_Pointer &t) {
    ...
    if (t == nullptr) {
        throw AVL_Tree_error("Värdet hittades inte");
    }
      ...
      //std::cout << "asdsadsad" << '\n';
        ...
        if (node_height(t->right) - node_height(t->left) == 2){
          if ((node_height(t->right->right) - node_height(t->right->left)) >= 0){
            //cout << "gjorde single" << endl;
            single_rotate_with_right_child(t);
          }
          else{
            //cout << "gjorde double" << endl;
            double_rotate_with_right_child(t);
          }
        }
        else{
        ...
        //std::cout << "dgfffffffff123123" << '\n';
        remove(x, t->right);
        if (node_height(t->left) - node_height(t->right) == 2){
          if ((node_height(t->left->left) - node_height(t->left->right)) >= 0){
            //1cout << "gjorde single left" <<endl;
            single_rotate_with_left_child(t);
          }
          else{
            //cout << "gjorde double left" << endl;
            double_rotate_with_left_child(t);
          }
        }
        else{
